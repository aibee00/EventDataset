...
def parse_vg_dataset_image_as_unit(label_path):
    ...
    for item in images:
        ...
        for anno in all_bbox_annotations:
            ...
            captions.append(f"{anno['caption']} ")
        caption = "\n".join(captions)
...
def convert_to_lavis_format(label_path, new_label_path, gen_caption_based_bbox=False):
    ...
    for idx, label in enumerate(tqdm(labels)):
        ...
        labels_llava.append({
            ...
            "image": image_id,
            "bbox": bbox,
            "dense_caption": f"{caption}{SEP}{bbox}"
        })
...
if __name__ == "__main__":
    use_based_on_each_bbox_description = True  # True: 基于每个bbox的描述，False: 基于图片的描述所有的boxes
    GEN_TRAIN = True  # True: 生成训练集，False: 生成测试集
    
    if GEN_TRAIN:
        convert_to_lavis_format(
            label_path="/training/wphu/Dataset/vg/annotations/train.json",
            new_label_path="/training/wphu/Dataset/vg/annotations/vg_for_lavis_train.json",
            gen_caption_based_bbox=use_based_on_each_bbox_description
        )
    else:
        convert_to_lavis_format(
            label_path="/training/wphu/Dataset/vg/annotations/test.json",
            new_label_path="/training/wphu/Dataset/vg/annotations/vg_for_lavis_test.json",
            gen_caption_based_bbox=use_based_on_each_bbox_description
        )
